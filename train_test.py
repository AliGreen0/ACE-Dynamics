...
def train(data_folder, modelpath, testonly):
    ...
    if 'BRCA' in data_folder:
        ...
        step_size = 500
        num_class = 5
        lambda_Adaptive_weighting = 0.0001
    ...
    model = MMDynamic(dim_list, hidden_dim, num_class, dropout=0.5, lambda_Adaptive_weighting=lambda_Adaptive_weighting)
    model.cuda()
    ...
    if testonly:
        ...
        if num_class == 2:
            test_accuacy = accuracy_score(labels_trte[trte_idx["te"]], te_prob.argmax(1))
        ...
        for epoch in range(num_epoch+1):
            ...
            if epoch % test_inverval == 0:
                ...
                if num_class == 2:
                    ...
                    test_f1 = f1_score(labels_trte[trte_idx["te"]], te_prob.argmax(1))
                    test_auc = roc_auc_score(labels_trte[trte_idx["te"]], te_prob[:,1])
                    print("Test ACC: {:.5f}".format(test_accuacy))
        ...
        if num_class == 2:
            return test_accuacy, test_f1, test_auc, num_class
        else:
            return test_accuacy, test_f1_weighted, test_f1_macro, num_class
